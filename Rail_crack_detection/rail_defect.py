...
def evaluate_model(model, scaler, X_test, y_test):
    """
    Evaluate the trained model using accuracy and other classification metrics.
    """
    y_pred = model.predict(X_test)
    accuracy = np.mean(y_pred == y_test)
    print(f"Model Accuracy: {accuracy * 100:.2f}%")
    
    # Save the model and scaler for later use (deployment)
    joblib.dump(model, 'rail_defect_model.pkl')
...
def main():
    ...
    for _ in range(100):  # Simulate 5 real-time samples
        ...
        print(f"Defect Detected: {'Yes' if prediction == 1 else 'No'}")
        time.sleep(1)
